...
function seilingByName (name) {
	...
	for (i = 0; i < seilingList.length; ++i) {
		...
		$('select[name="pvc-texture'+this.index+'"]').append('<option value="0">Не выбрана</option>');
...
// Прописываем список со светильниками
			...
			$('select[name*="light-type'+seilingList[j].index+'"]').append('<option value="0" data-price="0">Не выбран</option>');
			...
			for (i = 0; i < lamp.length; ++i) {
				...
				if ( c == 'firm1' ) {
					...
					s = s + lamp[i].title;
					s = s + '; ' + numberWithCommas(lamp[i].price1) + ' руб.';
				}
				else {
					s = s + ' data-price="' + numberWithCommas(lamp[i].price2) + '">';
					s = s + lamp[i].title;
					if ( lamp[i].price2 > 0 ) {
						s = s + '; ' + numberWithCommas(lamp[i].price2) + ' руб.';
					}
					else {
						s = s + '; в подарок при покупке лампы';
					}
				}
				s = s + '</option>';
...
// Прописываем список с лампами
			...
			$('select[name*="light-bulb'+seilingList[j].index+'"]').append('<option value="0" data-price="0">Не выбрана</option>');
			...
			for (i = 0; i < lamp.length; ++i) {
				...
				if ( c == 'firm1' ) {
					...
					s = s + light[i].title;
					s = s + '; ' + numberWithCommas(light[i].price1) + ' руб.';
				}
				else {
					s = s + ' data-price="' + numberWithCommas(light[i].price2) + '">';
					s = s + light[i].title;
					s = s + '; ' + numberWithCommas(light[i].price2) + ' руб.';
				}
				s = s + '</option>';
...
//	console.log(seilingList);
	...
	for (j = 0; j < seilingList.length; ++j) {
		...
		if (j % 2 == 0 && seilingList[j].type == 'ПВХ') {
			...
			seilingList[j].discountType = 'нечётный потолок';
		}
		...
		if (j % 2 == 1 && seilingList[j].type == 'ПВХ') {
			...
			seilingList[j].discountType = 'чётный потолок';
		}
...
Seiling.prototype.setTabTitle = function() {
	if (this.totalSum) {
		$('ul.nav-tabs li a#tab'+this.index).html( this.title + " • " + this.type + " • " + numberWithCommas(this.totalSum) );
	}
	else {
		$('ul.nav-tabs li a#tab'+this.index).html( this.title + " • " + this.type );
	}
}
...
function PVCSeiling() {
	...
	PVCSeiling.counter++;
	this.title = this.title ? this.title : 'Потолок ' + PVCSeiling.counter;
	this.type = 'ПВХ';
...
function TexSeiling() {
	...
	TexSeiling.counter++;
	this.title = this.title ? this.title : 'Потолок ' + TexSeiling.counter;
	this.type = 'Ткань';
...
// округление площади 0-25
	...
	for (i = 0; i < pvhTexture.length; ++i) {
		...
		if ( !(isNaN(area) ) && area > 0 ) {
			switch( true ) {
				case (area < 6):
					s = s + '; 1 м²: ' + numberWithCommas(pvhTexture[i].a0) + ' руб., итого: ' + numberWithCommas(pvhTexture[i].a0 * area) + ' руб.';
					break;
				case (area < 11):
					s = s + '; 1 м²: ' + numberWithCommas(pvhTexture[i].a5) + ' руб., итого: ' + numberWithCommas(pvhTexture[i].a5 * area) + ' руб.';
					break;
				case (area < 21):
					s = s + '; 1 м²: ' + numberWithCommas(pvhTexture[i].a10) + ' руб., итого: ' + numberWithCommas( pvhTexture[i].a10 * area) + ' руб.';
					break;
				default:
					s = s + '; 1 м²: ' + numberWithCommas(pvhTexture[i].a21) + ' руб., итого: ' + numberWithCommas( pvhTexture[i].a21 * area) + ' руб.';
			}
		}
...
PVCSeiling.prototype.calculateAreaPerimeter = function() {
	...
	if ( float( lengthVal ) > 0 && float( widthVal ) > 0 ) {
		...
		$('.perimeter-type').text('Периметр на основе длины и ширины, м:');
		...
		$('.perimeter-type').text('Периметр на основе площади, м:');
...
TexSeiling.prototype.updateTexture = function() {
	...
	$('select[name="tex-texture'+this.index+'"]').append('<option value="0">Не выбрана</option>');
	...
	for (i = 0; i < texTexture.length; ++i) {
		...
		if ( !(isNaN(area) ) && area > 0 ) {
			switch( true ) {
				case (area < 5):
					s = s + '; 1 м²: ' + numberWithCommas(texTexture[i].a0) + ' руб., итого: ' + numberWithCommas(texTexture[i].a0 * area) + ' руб.';
					break;
				case (area < 10):
					s = s + '; 1 м²: ' + numberWithCommas(texTexture[i].a5) + ' руб., итого: ' + numberWithCommas(texTexture[i].a5 * area) + ' руб.';
					break;
				case (area < 21):
					s = s + '; 1 м²: ' + numberWithCommas(texTexture[i].a10) + ' руб., итого: ' + numberWithCommas( texTexture[i].a10 * area) + ' руб.';
					break;
				default:
					s = s + '; 1 м²: ' + numberWithCommas(texTexture[i].a21) + ' руб., итого: ' + numberWithCommas( texTexture[i].a21 * area) + ' руб.';
			}
		}
...
// (3) Опеределяем, какая ориентация экономнее (для клиента) - в каком случае выкинутая площадь окажется меньше
		...
		if (portraitLoseArea > landscapeLoseArea) {
			...
			if (texLengthLandscape == length) {
				$('div#seiling' + this.index + ' .length-cut').html('(длина отреза)');
				$('div#seiling' + this.index + ' .width-cut').html('&nbsp;');
			}
			else {
				$('div#seiling' + this.index + ' .length-cut').html('&nbsp;');
				$('div#seiling' + this.index + ' .width-cut').html('(длина отреза)');
			}
