...
function seilingByName (name) {
	...
	for (i = 0; i < seilingList.length; ++i) {
		...
		$('select[name="pvc-texture'+this.index+'"]').append('<option value="0">Not selected</option>');
...
// Прописываем список со светильниками
			...
			$('select[name*="light-type'+seilingList[j].index+'"]').append('<option value="0" data-price="0">Not selected</option>');
...
// Прописываем список с лампами
			...
			$('select[name*="light-bulb'+seilingList[j].index+'"]').append('<option value="0" data-price="0">Not selected</option>');
...
//	console.log(seilingList);
	...
	for (j = 0; j < seilingList.length; ++j) {
		...
		if (j % 2 == 0 && seilingList[j].type == 'ПВХ') {
			...
			seilingList[j].discountType = 'odd ceiling';
		}
		...
		if (j % 2 == 1 && seilingList[j].type == 'ПВХ') {
			...
			seilingList[j].discountType = 'even ceiling';
		}
...
Seiling.prototype.setTabTitle = function() {
	...
	if (typeLabel == 'Ткань') { typeLabel = 'Fabric'; }
	if (this.totalSum) {
...
function PVCSeiling() {
	...
	PVCSeiling.counter++;
	this.title = this.title ? this.title : 'Ceiling ' + PVCSeiling.counter;
	this.type = 'ПВХ';
...
function TexSeiling() {
	...
	TexSeiling.counter++;
	this.title = this.title ? this.title : 'Ceiling ' + TexSeiling.counter;
	this.type = 'Ткань';
...
// округление площади 0-25
	...
	for (i = 0; i < pvhTexture.length; ++i) {
		...
		if ( !(isNaN(area) ) && area > 0 ) {
			switch( true ) {
				case (area < 6):
					s = s + '; 1 m²: ' + numberWithCommas(pvhTexture[i].a0) + ' $, total: ' + numberWithCommas(pvhTexture[i].a0 * area) + ' $';
					break;
				case (area < 11):
					s = s + '; 1 m²: ' + numberWithCommas(pvhTexture[i].a5) + ' $, total: ' + numberWithCommas(pvhTexture[i].a5 * area) + ' $';
					break;
				case (area < 21):
					s = s + '; 1 m²: ' + numberWithCommas(pvhTexture[i].a10) + ' $, total: ' + numberWithCommas( pvhTexture[i].a10 * area) + ' $';
					break;
				default:
					s = s + '; 1 m²: ' + numberWithCommas(pvhTexture[i].a21) + ' $, total: ' + numberWithCommas( pvhTexture[i].a21 * area) + ' $';
			}
		}
...
PVCSeiling.prototype.calculateAreaPerimeter = function() {
	...
	if ( float( lengthVal ) > 0 && float( widthVal ) > 0 ) {
		...
		$('.perimeter-type').text('Perimeter based on length and width, m:');
		...
		$('.perimeter-type').text('Perimeter based on area, m:');
...
TexSeiling.prototype.updateTexture = function() {
	...
	$('select[name="tex-texture'+this.index+'"]').append('<option value="0">Not selected</option>');
	...
	for (i = 0; i < texTexture.length; ++i) {
		...
		if ( !(isNaN(area) ) && area > 0 ) {
			switch( true ) {
				case (area < 5):
					s = s + '; 1 m²: ' + numberWithCommas(texTexture[i].a0) + ' $, total: ' + numberWithCommas(texTexture[i].a0 * area) + ' $';
					break;
				case (area < 10):
					s = s + '; 1 m²: ' + numberWithCommas(texTexture[i].a5) + ' $, total: ' + numberWithCommas(texTexture[i].a5 * area) + ' $';
					break;
				case (area < 21):
					s = s + '; 1 m²: ' + numberWithCommas(texTexture[i].a10) + ' $, total: ' + numberWithCommas( texTexture[i].a10 * area) + ' $';
					break;
				default:
					s = s + '; 1 m²: ' + numberWithCommas(texTexture[i].a21) + ' $, total: ' + numberWithCommas( texTexture[i].a21 * area) + ' $';
			}
		}
...
// (3) Опеределяем, какая ориентация экономнее (для клиента) - в каком случае выкинутая площадь окажется меньше
		...
		if (portraitLoseArea > landscapeLoseArea) {
			...
			if (texLengthLandscape == length) {
				$('div#seiling' + this.index + ' .length-cut').html('(cut length)');
				$('div#seiling' + this.index + ' .width-cut').html('&nbsp;');
			}
			else {
				$('div#seiling' + this.index + ' .length-cut').html('&nbsp;');
				$('div#seiling' + this.index + ' .width-cut').html('(cut length)');
			}
